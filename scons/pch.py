# Downloaded on 2013/02/17 from https://gist.github.com/larroy/1321295
# and modified to produce clang pch files rather than g++ gch ones.

# SCons builder for clang's precompiled headers
...
#  env['Pch'] = env.pch(target='common/includes/all.h.pch', source=env['precompiled_header'])
...
PchAction = SCons.Action.Action('$PCHCOM', '$PCHCOMSTR')
PchShAction = SCons.Action.Action('$PCHSHCOM', '$PCHSHCOMSTR')

def gen_suffix(env, sources):
    return sources[0].get_suffix() + env['PCHSUFFIX']

PchShBuilder = SCons.Builder.Builder(action = pchShAction,
                                     source_scanner = SCons.Scanner.C.CScanner(),
                                     suffix = gen_suffix)

PchBuilder = SCons.Builder.Builder(action = pchAction,
                                   source_scanner = SCons.Scanner.C.CScanner(),
                                   suffix = gen_suffix)

def header_path(node):
    h_path = node.abspath
    idx = h_path.rfind('.pch')
    if idx != -1:
        h_path = h_path[0:idx]
        ...
        raise SCons.Errors.StopError("{0} file doesn't have .pch extension".format(h_path))
...
def generate(env):
    """
    Add builders and construction variables for the Pch builder.
    """
    env.Append(BUILDERS = {
        'pch': env.Builder(
        action = PchAction,
    ...
    try:
        bld = env['BUILDERS']['Pch']
        bldsh = env['BUILDERS']['PchSh']
    except KeyError:
        bld = PchBuilder
        bldsh = PchShBuilder
        env['BUILDERS']['Pch'] = bld
        env['BUILDERS']['PchSh'] = bldsh

    env['PCHCOM']     = '$CXX -Wall -o $TARGET -x c++-header -c $CXXFLAGS $CCFLAGS $_CCCOMCOM $SOURCE'
    env['PCHSHCOM']   = '$CXX -o $TARGET -x c++-header -c $SHCXXFLAGS $CCFLAGS $_CCCOMCOM $SOURCE'
    env['PCHSUFFIX']  = '.pch'

    for suffix in SCons.Util.Split('.c .C .cc .cxx .cpp .c++'):
        static_pch_emitter = functools.partial(pch_emitter, 'Pch')
        env['BUILDERS']['StaticObject'].add_emitter( suffix, static_pch_emitter )
        shared_pch_emitter = functools.partial(pch_emitter, 'PchSh')
