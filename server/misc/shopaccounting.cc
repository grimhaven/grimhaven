                                  ...
                                  const sstring &name, int amt, bool new_id)
{
  ...
  queryqueue.push(format("insert into shoplogjournal (shop_nr, journal_id, customer_name, obj_name, sneezy_year, logtime, post_ref, debit, credit) values (%i, %s, '%s', '%s', %i, now(), %i, %i, 0)") % shop_nr % (sstring)(new_id?"NULL":"LAST_INSERT_ID()") % db.escape(customer) % db.escape(name) % GameTime::getYear() % post_ref % amt);
}
...
void TShopOwned::COGS_add(const sstring &name, int amt, int num)
{
  ...
  if(!db.fetchRow()){
    ...
    db.query("insert into shoplogcogs (shop_nr, obj_name, count, total_cost) values (%i, '%s', %i, %i)", shop_nr, num, amt);
    //    queryqueue.push(format("insert into shoplogcogs (shop_nr, obj_name, count, total_cost) values (%i, '%s', %i, %i)") % shop_nr % db.escape(name) % num % amt);
  } else {
    queryqueue.push(format("update shoplogcogs set count=count+%i, total_cost=total_cost+%i where obj_name='%s' and shop_nr=%i") % num % amt % db.escape(name) % shop_nr);
  }
}
...
void TShopOwned::COGS_remove(const sstring &name, int num)
{
  ...
  queryqueue.push(format("update shoplogcogs set total_cost=floor(total_cost-(total_cost/count)*%i), count=count-%i where obj_name='%s' and shop_nr=%i") % num % num % db.escape(name) % shop_nr);
}
