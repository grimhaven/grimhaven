  ...
  switch (cmd) {
    ...
    case CMD_RESP_CHECKLOAD:
      ...
      if (value <= 0 ||
        ...
        vlogf(LOG_MOB_RS, format("Problem in script (1).  Trying to load %d on %s") % value %getName());
        return FALSE;
      }
      if (!(obj = read_object(rc, REAL))) {
        vlogf(LOG_MOB_RS, format("Problem in script (2).  Trying to load %d on %s") % value %getName());
        return FALSE;
      }
      if (obj_index[rc].getNumber() > obj_index[rc].max_exist) {
        vlogf(LOG_MOB_RS, format("Quest mob (%s:%d) loading item (%s:%d) when over max_exist.") %
              getName() % mobVnum() %
      ...
      for(StuffIter it=stuff.begin();it!=stuff.end() && (t=*it);++it) {
        ...
        if (tob->number == obj->number) {
          ...
          if (cmd == CMD_LOAD) {
            ...
            vlogf(LOG_MOB_RS, format("Quest mob (%s) tried loading item (%s) that he already had.") %
                  getName() % obj->getName());
            delete obj;
    ...
    case CMD_RESP_LOADMOB:
      {
        ...
        if (value <= 0 ||
          ...
          vlogf(LOG_MOB_RS, format("Problem in script (3).  Trying to load %d on %s") % value %getName());
          return FALSE;
        }
        if (!(tMonster = read_mobile(rc, REAL))) {
          vlogf(LOG_MOB_RS, format("Problem in script (4).  Trying to load %d on %s") % value %getName());
          return FALSE;
        }
        if (mob_index[rc].getNumber() > mob_index[rc].max_exist) {
          vlogf(LOG_MOB_RS, format("Quest mob (%s:%d) loading mob (%s:%d) when over max_exist.") %
                getName() % mobVnum() %
                tMonster->getName() % tMonster->mobVnum());
        }
        if (mob_index[rc].spec == SPEC_SHOPKEEPER) {
          vlogf(LOG_MOB_RS, format("Problem in script.  %s trying to load %d which is a shopkeeper.") %
                getName() % value);
          delete tMonster;
          return FALSE;
        }
        if (mob_index[rc].spec == SPEC_NEWBIE_EQUIPPER) {
          vlogf(LOG_MOB_RS, format("Problem in script.  %s trying to load %d which is a newbie helper.") %
                getName() % value);
          delete tMonster;
          return FALSE;
        }
    ...
    case CMD_RESP_PERSONALIZE:
      ...
      if (value <= 0 || (real_object(value) < 0)) {
        vlogf(LOG_MOB_RS, format("Problem in script (5).  Trying to load %d on %s") %  value % getName());
        return FALSE;
      }
    ...
    case CMD_RESP_RESIZE:
      ...
      if (value <= 0 || (real_object(value) < 0)) {
        vlogf(LOG_MOB_RS, format("Problem in script (6).  Trying to load %d on %s") %  value % getName(\
));
        return FALSE;
      }
    ...
    case CMD_RESP_TOGGLE:
      ...
      if (value <= 0 || value >= MAX_TOG_INDEX) {
        vlogf(LOG_MOB_RS, format("Bad argument to response (%s) command %d.  (%s)") %
                name % cmd % arg);
        return FALSE;
      }
      if (TogIndex[value].togmob != Mob::ANY && TogIndex[value].togmob != mobVnum()) {
    ...
    case CMD_RESP_UNTOGGLE:
      value = convertTo<int>(arg);
      if (value <= 0 || value >= MAX_TOG_INDEX) {
        vlogf(LOG_MOB_RS, format("Bad argument to response (%s) special command %d.  (%s)") %
                name % cmd % arg);
        return FALSE;
      }
      if (!mob->hasQuestBit(value)) {
        vlogf(LOG_MOB_RS, format("Response file untoggling an unset bit: %d on %s by %s '%s'") %
    ...
    case CMD_RESP_CHECKMAX:
      ...
      if (value <= 0 || value >= (signed)obj_index.size()) {
        vlogf(LOG_MOB_RS, format("Bad argument to response (%s) special command %s.  (%s)") %
              name % cmd % arg);
        return FALSE;
      }
    ...
    case CMD_RESP_LINK:
      ...
      if (buf.lower() == "roomenter"){
        ...
        if (cmd_val >= MAX_CMD_LIST) {
          vlogf(LOG_MOB_RS, format("Responses::readCommand(): Parse error in %s. link bad.") %  buf);
          return RET_STOP_PARSING;
        }
      }
    ...
    switch(trig_cmd) {
      ...
      case CMD_GIVE:
        if (!(value = convertTo<int>(respo->args))) {
          vlogf(LOG_MOB_RS, format("Bad arguments for %s for Give") %  getName());
          break;
        }
        if (value > 0) {
          TObj *to = NULL;
      ...
      case CMD_GRIMACE:
       if ((isname( "none", respo->args) && (resp_targ == NULL)) ||
        ...
        break;
      default:
        vlogf(LOG_MOB_RS, format("Unknown command is checkResponse: %d") %  trig_cmd);
        break;
      } // end of switch
    }
  }
...
static void sstringTranslate(char *buf)
{
  ...
  while ((start_pt = strstr(buf, "#OBJCOST("))) {
    ...
    if (rc != 2) {
      vlogf(LOG_LOW, format("sstringTranslate(): failed parse OBJCOST (%d)") %  rc);
      return;
    }
    ...
    if (obj_index[robj].itemtype == ITEM_POTION) {
      ...
      if (obj) {
        if (pot) {
         ...
         price = 0;
          vlogf(LOG_LOW, format("Error casting object as potion in response script: %s(%d)" ) % obj->getName() % robj);
       }
        pot = NULL;
      } else {
        price = 0;
        vlogf(LOG_LOW, format("Error loading potion object in response script: %d" ) % robj);
      }
      delete obj;
      obj = NULL;
    } else {
...
resp * TMonster::readCommand(std::istringstream &is)
{
  ...
  cleanInputBuffer( is);
  if( (c=is.get()) != '{') {
    vlogf(LOG_LOW, format("Responses::readCommand(): Parse error in %s. Error after '%s'. Expected '{' but found '%c'") %
  ...
  if (!strcasecmp(cmdStr, "roomenter")) {
    ...
    if (cmd >= MAX_CMD_LIST) {
      vlogf(LOG_LOW, format("Responses::readCommand(): Parse error in %s. Unknown command %s.") %
            responseFile % cmdStr);
      return 0;
    }
  }
...
/*    cleanString( args);*/
    if (is_abbrev(buf, "toggle"))
      ...
      if (strstr(buf, "$N")) {
        vlogf(LOG_LOW, format("Found '$N' on toroom command inside response file for %s") %   responseFile);
        delete newResp;
        newResp = NULL;
        return 0;
      }
      ...
      if ((cmd=searchForCommandNum( buf)) >= MAX_CMD_LIST) {
        vlogf(LOG_MOB_RS,format("Responses::readCommand(): Parse error in %s. Unknown command %s.") %
              responseFile % buf);
        delete newResp;
        return 0;
      }
      if ((cmd == CMD_ECHO) ||
          (cmd == CMD_EMOTE)) {
        vlogf(LOG_MOB_RS, format("potential bad command (%s) in %s response") %  buf % getName());
        delete newResp;
        return 0;
      }
